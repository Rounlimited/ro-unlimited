...
import BeamDrop from '@/components/animations/BeamDrop';
...
export default function DivisionCards() {
  ...
  useGSAP(() => {
    ...
    // Refresh after all ScrollTriggers are set up in this component
              ...
              <Link key={division.id} href={division.href}
                ...
                <div className="p-8 sm:p-10">
                  <div className="flex items-start gap-4 mb-4">
                    <div className="division-icon w-12 h-12 border border-ro-gold/20 flex items-center justify-center text-ro-gold group-hover:bg-ro-gold/10 transition-colors flex-shrink-0">
                      <Icon size={24} />
                    </div>
                    <div>
                      <h3 className="division-name text-ro-white font-heading text-xl tracking-wider uppercase group-hover:text-ro-gold transition-colors">{division.name}</h3>
                  ...
                  <div className="pl-16 flex flex-wrap gap-2 mb-6">
                    {division.services.slice(0, 3).map((service) => (
                      <span key={service} className="division-tag px-3 py-1 text-xs font-mono text-ro-gray-500 border border-ro-gray-800 group-hover:border-ro-gold/20 group-hover:text-ro-gray-300 transition-colors">
                        {service}
                      </span>
                    ))}
                  </div>
                ...
                <div className="absolute bottom-0 left-0 right-0 h-[2px] bg-ro-gold/20">
                  <div className="division-bottom-line h-full bg-ro-gold w-full" />
                </div>
              </Link>
            );
          })}
