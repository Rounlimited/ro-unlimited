...
import {
  ...
  ChevronDown, HardHat, Wrench, DollarSign, Clock,
} from 'lucide-react';
import { useEffect, useState, useRef } from 'react';
import { gsap, ScrollTrigger } from '@/components/animations/GSAPProvider';

export default function RoofingSubServicePage({ subService }: { subService: RoofingSubService }) {
  ...
  const otherServices = ROOFING_SUB_SERVICES.filter(s => s.id !== subService.id);
  ...
  const jsonLd = {
    ...
    url: `https://rounlimited.com/services/roofing/${subService.slug}`,
    provider: {
      '@type': 'LocalBusiness',
  ...
  const breadcrumbJsonLd = {
    ...
    itemListElement: [
      ...
      { '@type': 'ListItem', position: 3, name: 'Roofing', item: 'https://rounlimited.com/services/roofing' },
      { '@type': 'ListItem', position: 4, name: subService.title, item: `https://rounlimited.com/services/roofing/${subService.slug}` },
    ],
  };
  ...
  return (
    <div ref={containerRef}>
        ...
        <div className="relative z-10 px-6 sm:px-10 lg:pl-16 pt-28 pb-16" style={{ maxWidth: 'min(620px, 100%)' }}>
          <nav aria-label="Breadcrumb" className="mb-5">
            <ol className="flex flex-wrap items-center gap-x-2 gap-y-1 text-[10px] font-mono tracking-wider uppercase text-ro-gray-400">
              ...
              <li className="text-ro-gold/30">/</li>
              <li><Link href="/services/roofing" className="hover:text-ro-gold transition-colors">Roofing</Link></li>
              <li className="text-ro-gold/30">/</li>
          ...
          <div className="hero-badge inline-flex items-center gap-2 px-4 py-1.5 border border-ro-gold/25 bg-ro-black/40 backdrop-blur-sm mb-7 self-start">
            <HardHat size={12} className="text-ro-gold flex-shrink-0" />
            <span className="text-ro-gold text-[10px] font-mono tracking-[0.3em] uppercase">RO Roofing — {subService.title}</span>
          </div>
      ...
      <section ref={crossRef} className="py-28 sm:py-36 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            <span className="text-ro-gold text-xs font-mono tracking-[0.4em] uppercase block mb-4">More Roofing Services</span>
            <h2 className="text-ro-white font-heading text-3xl sm:text-4xl tracking-tight uppercase">
              ...
              <Link key={svc.id} href={`/services/roofing/${svc.slug}`}
                ...
                <div className="relative z-10">
                  <div className="w-9 h-9 mx-auto flex items-center justify-center border border-ro-gold/15 bg-ro-gold/[0.04] mb-4 group-hover:border-ro-gold/35 transition-all duration-700">
                    <HardHat size={16} className="text-ro-gold/60 group-hover:text-ro-gold transition-colors duration-700" />
                  </div>
            ...
            {otherServices.map((svc) => (
              <Link key={svc.id} href={`/services/roofing/${svc.slug}`}
                ...
                <div className="w-9 h-9 mx-auto flex items-center justify-center border border-ro-gold/15 bg-ro-gold/[0.04] mb-3">
                  <HardHat size={16} className="text-ro-gold/60" />
                </div>
          ...
          <div className="mt-10 text-center">
            <Link href="/services/roofing" className="inline-flex items-center gap-2 text-ro-gold/50 text-sm font-mono tracking-wider uppercase hover:text-ro-gold transition-colors">
              All Roofing Services <ArrowRight size={14} />
            </Link>
          </div>
        </div>
      </section>
