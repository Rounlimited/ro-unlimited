...
export async function POST(req: NextRequest) {
  ...
  const to_email = parseEmail(typeof to === 'string' ? to : Array.isArray(to) ? to[0] : 'noreply@rounlimited.com');
  ...
  // Forward a copy to Gmail
  const forwardTo = process.env.EMAIL_FORWARD_TO || 'Rounlimitedco@gmail.com';
  try {
    await getResend().emails.send({
      from: 'RO Unlimited Inbox <noreply@rounlimited.com>',
      to: [forwardTo],
      subject: `[Inbound] ${subjectStr}`,
      html: body_html
        ? `<p style="color:#888;font-size:12px;margin-bottom:16px">From: ${from}<br>To: ${to_email}</p>${body_html}`
        : `<p style="color:#888;font-size:12px">From: ${from}</p><pre>${body_text || ''}</pre>`,
      replyTo: from_email,
    });
  } catch (err) {
    console.error('Gmail forward failed (non-fatal):', err);
  }
