...
export default function ResidentialPage() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // Hero — video plays first, then elements build bottom-up (warm, softer than commercial)
      if (heroRef.current) {
      ...
      <section ref={heroRef} className="relative min-h-[100vh] flex flex-col overflow-hidden">
        {videoUrl ? (
          <>
            ...
            {/* Left-heavy warm gradient — right side of video stays vivid */}
            <div className="absolute inset-0" style={{ zIndex: 1, background: 'linear-gradient(to right, rgba(20,14,8,0.97) 0%, rgba(20,14,8,0.88) 28%, rgba(15,10,5,0.5) 50%, rgba(0,0,0,0.0) 70%)' }} />
            {/* Top/bottom vignette */}
            <div className="absolute inset-0" style={{ zIndex: 1, background: 'linear-gradient(to bottom, rgba(15,10,5,0.7) 0%, transparent 18%, transparent 72%, rgba(10,8,5,0.9) 100%)' }} />
            <div className="absolute inset-0 blueprint-overlay-warm opacity-8" style={{ zIndex: 1 }} />
          </>
        ) : (
          <>
        ...
        {/* ── Vertical warm gold rule — desktop only ── */}
        <div className="hidden lg:block absolute top-0 bottom-0 w-px bg-gradient-to-b from-transparent via-ro-gold/20 to-transparent" style={{ left: 'min(460px, 36vw)', zIndex: 3 }} />
        ...
        <div className="relative z-10 flex flex-col justify-center flex-1 px-6 sm:px-10 lg:pl-16 lg:pr-0 pt-28 pb-6" style={{ maxWidth: 'min(480px, 100%)' }}>
          <div className="hero-badge inline-flex items-center gap-2 px-4 py-1.5 border border-ro-gold/25 bg-ro-gold/[0.06] backdrop-blur-sm mb-7 self-start">
            <Home size={12} className="text-ro-gold flex-shrink-0" />
            <span className="text-ro-gold text-[10px] font-mono tracking-[0.3em] uppercase">Residential Division</span>
          </div>
          ...
          <div className="hero-btns flex flex-col sm:flex-row lg:flex-col xl:flex-row gap-3">
            <Link href="/contact" className="group inline-flex items-center gap-2 px-6 py-3 bg-ro-gold text-ro-black font-heading text-xs tracking-[0.15em] uppercase hover:bg-ro-gold-light transition-all duration-300 whitespace-nowrap">
              Start Your Build <ArrowRight size={13} className="group-hover:translate-x-1 transition-transform" />
            </Link>
        ...
        <div className="hero-stats relative z-10 mt-auto">
          <div className="w-full">
            <div className="grid grid-cols-2 sm:grid-cols-4 divide-x divide-ro-gold/10 border-t border-ro-gold/10"
              style={{ background: 'rgba(12,8,4,0.85)', backdropFilter: 'blur(12px)' }}>
              {HERO_STATS.map((s, i) => (
                <div key={i} className="px-6 py-4 text-center">
                  <div className="font-heading text-xl sm:text-2xl text-ro-gold tracking-tight">{s.value}{s.suffix}</div>
