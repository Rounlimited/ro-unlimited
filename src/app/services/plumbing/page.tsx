...
import { ArrowRight, Phone, Pipette, ChevronDown } from 'lucide-react';
...
const CROSS_DIVISIONS = [
  ...
  { id: 'electrical', label: 'Electrical Services', desc: 'Panel upgrades, EV chargers, solar & more', href: '/services/electrical' },
  { id: 'septic', label: 'Septic Systems', desc: 'Installation, repair & maintenance', href: '/services/septic' },
];
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      if (guidesRef.current) {
        ...
        guidesRef.current.querySelectorAll('.guide-card').forEach((c, i) => {
      ...
      if (crossRef.current) {
        gsap.fromTo(crossRef.current.querySelectorAll('.cross-card'), { y: 35, opacity: 0 },
  ...
  return (
    <div ref={containerRef}>
      ...
      <section ref={heroRef} className="relative min-h-[100vh] flex flex-col overflow-hidden">
        <Image src={category.heroImage} alt="RO Unlimited Plumbing Services" fill priority className="object-cover" sizes="100vw" style={{ zIndex: 0 }} />
        ...
        <div className="relative z-10 flex flex-col justify-center flex-1 px-6 sm:px-10 lg:pl-16 pt-28 pb-6" style={{ maxWidth: 'min(520px, 100%)' }}>
          <div className="hero-badge inline-flex items-center gap-2 px-4 py-1.5 border border-ro-gold/25 bg-ro-black/40 backdrop-blur-sm mb-7 self-start">
            <Pipette size={12} className="text-ro-gold flex-shrink-0" />
            <span className="text-ro-gold text-[10px] font-mono tracking-[0.3em] uppercase">RO Plumbing Division</span>
          </div>
          <h1 className="font-heading uppercase leading-[0.88] tracking-tight mb-6" style={{ fontSize: 'clamp(2.6rem, 5vw, 4.5rem)' }}>
            <span className="hero-text-fade">Fast Response.</span><br />
            <span className="gradient-text-gold">Honest Fix.</span><br />
            <span className="hero-text-fade">No Excuses.</span>
          </h1>
          <div className="hero-gold-line w-10 h-[2px] bg-gradient-to-r from-ro-gold/80 to-transparent mb-6" />
          <p className="hero-desc text-sm sm:text-base leading-relaxed mb-8 max-w-sm hero-text-fade-subtle">{category.hero}</p>
      ...
      <section ref={guidesRef} className="py-28 sm:py-36 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="section-head mb-20 text-center">
            ...
            <h2 className="text-ro-white font-heading text-4xl sm:text-5xl lg:text-6xl tracking-tight uppercase leading-[0.9]">Plumbing<br /><span className="gradient-text-gold">Services</span></h2>
            <p className="text-ro-gray-400 text-sm sm:text-base mt-6 max-w-lg mx-auto leading-relaxed">Tap into any specialty below for detailed guides — costs, process, warning signs, and exactly how we do the work.</p>
            <div className="w-20 h-[2px] bg-gradient-to-r from-transparent via-ro-gold/40 to-transparent mx-auto mt-6" />
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
            {PLUMBING_SUB_SERVICES.map((svc) => (
              <Link key={svc.id} href={`/services/plumbing/${svc.slug}`} className="guide-card group relative overflow-hidden border border-ro-gray-800/40 hover:border-ro-gold/25 transition-all duration-700">
                ...
                <div className="relative z-10 flex items-center gap-5 p-5 sm:p-7 sm:flex-col sm:text-center">
                  <div className="w-12 h-12 sm:w-11 sm:h-11 flex-shrink-0 flex items-center justify-center border border-ro-gold/25 sm:border-ro-gold/15 bg-ro-black/50 sm:bg-ro-gold/[0.04] sm:mb-2 group-hover:border-ro-gold/35 group-hover:bg-ro-black/50 transition-all duration-700">
                    <Pipette size={20} className="text-ro-gold sm:text-ro-gold/70 sm:group-hover:text-ro-gold transition-colors duration-700" />
                  </div>
                  <div className="flex-1 sm:flex-initial">
                    ...
                    <div className="hidden sm:flex items-center justify-center gap-2 text-ro-gold/40 text-xs font-mono tracking-wider uppercase group-hover:text-ro-gold transition-colors duration-500">Read Guide <ArrowRight size={12} /></div>
                  </div>
      ...
      <section ref={galleryRef} className="py-24 sm:py-32 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            ...
            <h2 className="text-ro-white font-heading text-3xl sm:text-4xl tracking-tight uppercase">Plumbing <span className="gradient-text-gold">in Action</span></h2>
            <div className="w-20 h-[2px] bg-gradient-to-r from-transparent via-ro-gold/40 to-transparent mx-auto mt-6" />
          </div>
            ...
            {category.galleryImages.map((img, i) => (
              <div key={i} className={`gallery-item group relative overflow-hidden border border-ro-gold/10 ${i === 0 ? 'lg:row-span-2' : ''}`}>
                <div className={`relative ${i === 0 ? 'aspect-[3/4]' : 'aspect-[4/3]'}`}>
                  <Image src={img} alt="RO Unlimited plumbing work" fill className="object-cover group-hover:scale-105 transition-transform duration-1000" sizes="(max-width: 1023px) 100vw, 33vw" />
            ...
            {category.galleryImages.map((img, i) => (
              <div key={i} className="gallery-item flex-shrink-0 w-[75vw] snap-center">
                <div className="relative aspect-[4/3] overflow-hidden border border-ro-gold/10">
                  <Image src={img} alt="RO Unlimited plumbing work" fill className="object-cover" sizes="(max-width: 1023px) 100vw, 33vw" />
                </div>
              </div>
            ))}
          </div>
      ...
      <section ref={faqRef} className="py-28 sm:py-36 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            ...
            <h2 className="text-ro-white font-heading text-3xl sm:text-4xl tracking-tight uppercase"><span className="gradient-text-gold">FAQ</span></h2>
            <div className="w-20 h-[2px] bg-gradient-to-r from-transparent via-ro-gold/40 to-transparent mx-auto mt-6" />
          </div>
          <div className="space-y-4">
            {category.faq.map((item, i) => (
              <div key={i} className="faq-item border border-ro-gray-800/40 bg-[#1a150d]/20 backdrop-blur-sm hover:border-ro-gold/15 transition-colors duration-500">
                <button onClick={() => setExpandedFaq(expandedFaq === i ? null : i)} className="w-full flex items-start gap-4 p-5 sm:p-6 text-left cursor-pointer group">
                  <div className="flex-1"><h3 className="text-ro-white font-heading text-sm sm:text-base tracking-wider uppercase group-hover:text-ro-gold-light transition-colors">{item.q}</h3></div>
                ...
                {expandedFaq === i && (<div className="px-5 sm:px-6 pb-5 sm:pb-6 -mt-2"><p className="text-ro-gray-300 text-sm leading-relaxed">{item.a}</p></div>)}
              </div>
            ))}
          </div>
        </div>
      ...
      <section ref={crossRef} className="py-28 sm:py-36 relative overflow-hidden">
        ...
        <div className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-16">
            ...
            <h2 className="text-ro-white font-heading text-3xl sm:text-4xl tracking-tight uppercase">More Than <span className="gradient-text-gold">Plumbing</span></h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6">
            {CROSS_DIVISIONS.map((div) => (
              <Link key={div.id} href={div.href} className="cross-card group relative p-8 border border-ro-gray-800/40 bg-[#1a150d]/20 hover:border-ro-gold/25 hover:bg-ro-gold/[0.03] transition-all duration-700 text-center">
                ...
                <div className="mt-4 flex items-center justify-center gap-1 text-ro-gold/40 text-xs font-mono tracking-wider uppercase opacity-0 group-hover:opacity-100 transition-opacity duration-500">Explore <ArrowRight size={12} /></div>
              </Link>
            ))}
          </div>
        </div>
      ...
      <section ref={ctaRef} className="relative min-h-[60vh] flex items-center justify-center overflow-hidden">
        ...
        <div className="relative z-10 max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <div className="cta-inner">
            ...
            <span className="text-ro-gold/70 text-xs font-mono tracking-[0.4em] uppercase block mb-6">Need a Plumber?</span>
            <h2 className="text-ro-white font-heading text-4xl sm:text-5xl lg:text-6xl tracking-tight uppercase leading-[0.85] mb-8">Call<br /><span className="gradient-text-gold">Right Now</span></h2>
            <p className="text-ro-gray-300 text-sm sm:text-base leading-relaxed mb-10 max-w-md mx-auto">No hourly padding, no surprise charges. Just honest work at a fair price from licensed plumbers who show up on time.</p>
            <a href={`tel:${COMPANY.phone.replace(/[^0-9]/g, '')}`} className="inline-flex items-center gap-4 text-ro-gold font-heading text-3xl sm:text-4xl tracking-tight hover:text-ro-gold-light transition-colors duration-300 mb-10">
